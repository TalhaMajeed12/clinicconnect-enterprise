...
3. Copy `.env.example` to `.env` and replace every placeholder. Keep PostgreSQL in `DATABASE_URL`; generate a Fernet key with `Fernet.generate_key()` for `ENCRYPTION_KEY`.
...
- Clinician: dashboard, patients, visits, appointments, and attendance.
- Patient: registration/login, dashboard, history/prescriptions, booking, cancellation, and demo payment records.
...
`render.yaml`, `Procfile`, `wsgi.py`, and `gunicorn.conf.py` define deployment. Configure `DATABASE_URL`, `ENCRYPTION_KEY`, and all required secrets in Render. The temporary production configuration uses one Gunicorn worker because filesystem sessions are not shared; configure Redis before scaling workers.
